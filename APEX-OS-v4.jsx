...
const parseCEOCommands = (text) => {
  const result = { tasks: [], assigns: [], memories: [], kpis: [], notifications: [], reports: [] };
  const patterns = {
    tasks: /\[TASK:(\{.*?\})\]/g,
    assigns: /\[ASSIGN:(\{.*?\})\]/g,
    memories: /\[MEMORY:(\{.*?\})\]/g,
...
// ── JSON CLAUDE CALL ─────────────────────────────────────
const callClaudeJSON = async (messages, system) => {
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: "11px 20px", borderBottom: `1px solid ${T.border}`, background: T.surf, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div style={{ background: `${T.green}15`, color: T.green, padding: "4px 12px", borderRadius: 20, fontSize: "0.72rem", fontWeight: 600 }}>● APEX Online</div>
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          ...
          {view === "settings" && (
            <div style={{ padding: 18, maxWidth: 640 }}>
              ...
              <div style={{ background: T.surf, border: `1px solid ${T.border2}`, borderRadius: 14, padding: 20, marginBottom: 16 }}>
                <div style={{ fontWeight: 700, fontSize: "0.88rem", marginBottom: 10 }}>🔌 AI Backend</div>
                <div style={{ fontSize: "0.78rem", color: T.text2, lineHeight: 1.6 }}>
                  {PROXY_BASE_URL ? <>Routing through proxy: <span style={{ color: T.green }}>{PROXY_BASE_URL}</span></> : <>No proxy configured — calling Claude API directly.</>}
                </div>
              </div>
